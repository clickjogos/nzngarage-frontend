...
export default class Competitors extends Component {
		...
		try {
			...
			if (keyWordsList.length > 0) {
				...
				keyWordsList.map((element) => {
					element.competitors.map((c) => {
						c.checked = false
						c.show = false
						element.competitorPosition = c.competitorPosition

						if (preSelectedKeywords.length > 0) {
							console.log('preSelectedKeywords ', preSelectedKeywords)
							preSelectedKeywords.forEach((kwSelected) => {
								if (kwSelected.Keyword == element.Keyword) {
									kwSelected.competitors.map((kwSelectCompetitor) => {
										if (kwSelectCompetitor.competitor == c.competitor) {
											if (kwSelectCompetitor.checked) {
												c.checked = true
												c.show = true
											}
										}
				...
				this.setState({ keyWordsList, page, loading: false })
			} else {
				this.setState({ keyWordsList: null, loading: false })
	...
	keyWordCheck = (checked, _id) => {
                ...
                kwSelected.competitors.map((kwSelectCompetitor) => {
                  if (kwSelectCompetitor.competitor == c.competitor) {
                    if (kwSelectCompetitor.checked) {
                      ...
                      kwSelectCompetitor.checked = true
                      pushed = true
                    }
                  }
                })
						...
						if (!pushed) preSelectedKeywords.push(element)
					} else {
		...
		this.setState({ keyWordsList, preSelectedKeywords, buttonKeywordDisabled })
	}
	...
	actionPage = async (status) => {
		let page = this.state.page
		if (!status && page.currentPage !== 1) page.currentPage -= 1
		if (status && page.currentPage < page.totalPages) page.currentPage += 1
		this.setState({ page }, async (r) => {
			await this.allKeywordsList()
		})
	}

	actionChangePage = async (currentPage) => {
		let page = this.state.page
		...
		<div className="container-flex">
			<div className="container-competitors">
				...
				<table>
					<thead>
						<tr>
							...
							<th onClick={() => this.orderArrayBy('competitorPosition')}>
								Pos. Concorrente <img style={this.state.orderArray.pos ? { transform: 'rotate(180deg)' } : null} className="chevron" src={Chevron} />
							</th>
							<th onClick={() => this.orderArrayBy('nznPosition')}>
								Pos. NZN <img style={this.state.orderArray.posNzn ? { transform: 'rotate(180deg)' } : null} className="chevron" src={Chevron} />
							</th>
					...
					<tbody>
						{this.state.keyWordsList ? (
							this.state.keyWordsList.map((e) => (
								<React.Fragment>
									<tr>
										<td onClick={() => this.actionCollapse(e)}>
											<img style={e.show ? { transform: 'rotate(0deg)' } : null} id="toggleImage" src={Chevron} />
										</td>
										...
										<td>{e.nznPosition}</td>
										<td>
											<p data-tip={e.competitors[0].competitorInfo.title}>
												{e.competitors[0].competitorInfo.title.substring(0, 29)}
												{e.competitors[0].competitorInfo.title.length > 29 && '...'} <img src={Tooltip} />{' '}
											</p>{' '}
										</td>
										<td>
											<a target="_blank" href={e.competitors[0].competitorInfo.Url}>
												{e.competitors[0].competitorInfo.Url} ↗
											</a>
										</td>
										...
										(c) =>
											e.show && (
												<tr key={c._id}>
													<td></td>
													<td>
														<div onClick={() => this.keyWordCheck(c.checked, c._id)} id="checkbox">
															{' '}
															{c.checked && <span></span>}{' '}
														</div>
													</td>
													...
													<td>{c.nznPosition}</td>
													<td>
														<p data-tip={c.competitorInfo.title}>
															{c.competitorInfo.title.substring(0, 29)}
															{c.competitorInfo.title.length > 29 && '...'} <img src={Tooltip} />{' '}
														</p>{' '}
													</td>
													<td>
														<a target="_blank" href={c.competitorInfo.Url}>
															{c.competitorInfo.Url} ↗
														</a>
													</td>
													<td onClick={() => this.openModalConfirm(e, c)}>
														<img src={Trash} />
													</td>
												</tr>
											)
				...
				<div className="container-send-button">
					<Button callback={() => this.sendToReview()} disabled={this.state.buttonKeywordDisabled} title="Continuar >" />
				</div>
			</div>
	...
	showReview = () => (
		<div className="container-flex">
			<div className="container-competitors">
					...
					<tbody>
						{this.state.keyWordsSelected.map((e) => (
							<React.Fragment>
								<tr>
									<td>
										<div onClick={() => this.keyWordCheckSelect(e.checked, e._id)} id="checkbox">
											{' '}
											{e.checked && <span></span>}{' '}
										</div>
									</td>
									...
									<td>{e.nznPosition}</td>
									<td>
										<p data-tip={e.competitorInfo.title}>
											{e.competitorInfo.title.substring(0, 29)}
											{e.competitorInfo.title.length > 29 && '...'}{' '}
										</p>{' '}
									</td>
									<td>
				...
				<div className="container-send-button">
					<Button disabled={this.state.buttonSuggestionDisabled} callback={() => this.sendKeywords()} title="Enviar para sugestão de produção ❯" />
				</div>
			</div>
		</div>
	)
	...
	render() {
		...
		return (
			<div className="refine-planning-main">
				<Sidebar></Sidebar>
				{!this.state.loading ? <React.Fragment>{this.state.review ? this.showReview() : this.showCompetitorKeyWords()}</React.Fragment> : <Loading />}
			</div>
		)
	}
}
